	...
	<div class="addEdit-block" :style='{"padding":"85px 30px 30px","fontSize":"14px","color":"#666","background":"none"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"border":"0px solid #eee","padding":"0px 0","margin":"0 0 20px 0","color":"inherit","borderRadius":"0px","background":"none","width":"100%","fontSize":"inherit"}' class="input" v-if="type!='info'"  label="续住天数" prop="xuzhutianshu">
					<el-input v-model.number="ruleForm.xuzhutianshu" placeholder="续住天数" clearable  :readonly="ro.xuzhutianshu"></el-input>
				</el-form-item>
				<el-form-item :style='{"border":"0px solid #eee","padding":"0px 0","margin":"0 0 20px 0","color":"inherit","borderRadius":"0px","background":"none","width":"100%","fontSize":"inherit"}' v-else class="input" label="续住天数" prop="xuzhutianshu">
					<el-input v-model="ruleForm.xuzhutianshu" placeholder="续住天数" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"border":"0px solid #eee","padding":"0px 0","margin":"0 0 20px 0","color":"inherit","borderRadius":"0px","background":"none","width":"100%","fontSize":"inherit"}' v-if="type!='info'"  label="备注" prop="beizhu">
					<editor 
				...
				<el-form-item :style='{"border":"0px solid #eee","padding":"0px 0","margin":"0 0 20px 0","color":"inherit","borderRadius":"0px","background":"none","width":"100%","fontSize":"inherit"}' v-else-if="ruleForm.beizhu" label="备注" prop="beizhu">
                    <span :style='{"fontSize":"14px","lineHeight":"40px","minWidth":"400px","color":"inherit","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.beizhu"></span>
                </el-form-item>
...
export default {
	data() {
		...
		return {
			...
			rules: {
				...
				xuzhutianshu: [
					{ required: true, message: '续住天数不能为空', trigger: 'blur' },
	...
	computed: {
		...
		zongjine:{
			get: function () {
				return 1*this.ruleForm.kefangjiage*this.ruleForm.xuzhutianshu
			}
		},
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='xuzhutianshu'){
							this.ruleForm.xuzhutianshu = obj[o];
							this.ro.xuzhutianshu = true;
							continue;
						}
						if(o=='zongjine'){
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.beizhu = this.ruleForm.beizhu.replace(reg,'../../../phpci7firyg/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `xuzhudengji/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.xuzhudengjiCrossAddOrUpdateFlag = false;
											this.parent.search();
