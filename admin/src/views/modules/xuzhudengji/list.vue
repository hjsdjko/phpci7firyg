		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"0px solid #bababa","padding":"0px","margin":"0px","overflow":"hidden","borderRadius":"0px","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"border":"0px solid #e3e3e3","padding":"0px","margin":"20px 0 10px","color":"#fff","textAlign":"left","flexDirection":"row","display":"flex","justifyContent":"flex-start","borderRadius":"5px","flexWrap":"wrap","background":"none","width":"100%","fontSize":"inherit"}'>
					<el-button class="add" v-if="isAuth('xuzhudengji','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('xuzhudengji','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
					...
					<el-button class="statis" v-if="isAuth('xuzhudengji','报表')" type="success" @click="chartDialog()">
						<span class="icon iconfont icon-tongji5" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						统计报表
					</el-button>
				</el-row>
			</el-form>
			<div :style='{"border":"1px solid #e9e9e9","width":"100%","padding":"20px","margin":"10px 0 0","borderRadius":"5px","background":"rgba(255,255,255,1)"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('xuzhudengji','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('xuzhudengji','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('xuzhudengji','删除') " type="primary" @click="deleteHandler(scope.row.id )">
...
import AddOrUpdate from "./add-or-update";
    ...
    chartDialog() {
		...
		this.$nextTick(()=>{
			...
			//  zongjine
        // xcolumn ycolumn kefangmingcheng zongjine
        var zongjineChart = echarts.init(document.getElementById("zongjineChart"),'macarons');
        this.$http({
            url: `xuzhudengji/value/kefangmingcheng/zongjine`,
            method: "get"
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
	...
	xuzhudengjistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'xuzhudengji/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
